...
----------------------------------------------------
createSidebarTab("Welcome", "rbxassetid://10822165440", 1)
createSidebarTab("Client", "rbxassetid://10723345479", 2)
createSidebarTab("Core Settings", "rbxassetid://10734951111", 3)

-- Welcome Tab Content
createToggle("Welcome", "Show Intro", "Display welcome greetings on configuration execution.", function(state)
    print("Welcome Intro Set:", state)
end)

-- Client Tab Content (Dito ko nilagay yung Teleport Buttons mo)
createToggle("Client", "Full Bright", "Make the game brighter, easier to see or look around.", function(state)
    print("Full Bright toggled:", state)
end)

-- HALIMBAWA NG MGA ONE-TAP BUTTONS PARA SA TELEPORT AT CFRAME:
createButton("Client", "Teleport to Spawn", "Instant teleportation of character straight to the map's default spawn point.", function()
    local character = localPlayer.Character
    local rootPart = character and character:FindFirstChild("HumanoidRootPart")
    if rootPart then
        rootPart.CFrame = CFrame.new(0, 10, 0) -- Baguhin mo rito ang coordinates ng pupuntahan
        print("Successfully teleported to position!")
    end
end)

createButton("Client", "Set CFrame Forward", "Move or shift your character's current position 10 studs forward.", function()
    local character = localPlayer.Character
    local rootPart = character and character:FindFirstChild("HumanoidRootPart")
    if rootPart then
        rootPart.CFrame = rootPart.CFrame * CFrame.new(0, 0, -10) -- Mag-mu-move ng 10 studs pasulong
        print("CFrame updated forward!")
    end
end)

createToggle("Client", "Float", "Make your character float in the air.", function(state)
    print("Float toggled:", state)
end)
createToggle("Client", "Float", "Make your character float in the air.", function(state)
    print("Float toggled:", state)
end)
...
createButton("Core Settings", "Export Configs", "Copy config to your clipboard", function()
    if setclipboard then
        setclipboard("Universal_Config_Active = true")
        print("Copied to clipboard!")
    else
        print("Your executor does not support clipboard functions.")
    end
end)
